...
export default function ProductDetailContent() {
  ...
  const showAlert = (quantitly) => {
    Swal.fire({
      icon: 'error',
      text: `Số lượng còn lại của sản phẩm này là: ${quantitly}`,
    })
  }
  function add() {
    if (
      productDetail.quantityChoose < productDetail.quantity &&
      T < productDetail.quantity
    ) {
      setT((T) => T + 1)
    } else if ((productDetail.quantityChoose = productDetail.quantity)) {
  ...
  const handleAddToCart = () => {
    ...
    if (!userInfo?.user?.name) {
      navigate('/sign-in')
    } else if (productDetail.quantityChoose)
