...
export default function ProductDetailContent() {
  ...
  const productTemp= cartItems.find((item) => item._id === productDetail._id)
  console.log(productTemp)
  const [T, setT] = useState(1)
  const showAlert = () => {
    Swal.fire({
      icon: 'error',
      text: `Số lượng còn lại của sản phẩm này là: ${productDetail.quantity}`,
    })
  }
  function add() {
    if (T < productDetail.quantity) {
      setT((T) => T + 1)
    }
  }
  function sub() {
  ...
  const handleAddToCart = () => {
    ...
    if (!userInfo?.user?.name) {
      navigate('/sign-in')
    } else dispatch(addToCart({ ...productDetail, quantityChoose: T }))
  }
    ...
    <div className='flex w-full bg-white'>
      <div className='flex flex-col w-full px-4 overflow-hidden bg-white select-none mx-[73px] border-b border-dashed border-[#dcdcdc]'>
        <div className='flex bg-white rounded w-full'>
          {/* left detail*/}
          <div className=' grow-2 relative p-6 bg-white border-r border-dashed border-[#dcdcdc] w-full'>
            {/* hình to */}
