...
export default function RightbarEditAddress() {
    ...
    const { address } = useSelector((state) => state.deliveryInfo);
        ...
        <div className="mr-32 basis-10/12 h-fit pb-12" >
            ...
            <form onSubmit={handleSubmit(onSubmit)} className='bg-white h-fit py-[30px] px-[20px]'>
                ...
                <div className='flex items-center flex-1 justify-between w-[600px] mb-[15px]'>
                    ...
                    <input control={control} name='Phone' {...register("Phone")}error={errors.name?.message} required className='py-[6px] px-[12px] w-[490px] text-[14px] border border-zinc-300 rounded' placeholder='Nhập số điện thoại' 
