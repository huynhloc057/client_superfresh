...
function ShippingForm() {
  ...
  const { address: deliveryInfo} = useSelector((state) => state.address);
  ...
  const handleSubmitAddress = (dataAddress) => {
    ...
    dispatch(
      addDeliveryInfo({ ...dataAddress, ...temp })
    );
    reset();
    navigate("/cart");
  };
  ...
  return (
    <div className={clsx("max-w-[79rem]", "m-auto", "mt-6", "mb-6")}>
      {/* <ContentHeader /> */}
      <div className="flex items-center justify-center py-3 pt-10 pb-10 border border-dashed border-[#dcdcdc]">
