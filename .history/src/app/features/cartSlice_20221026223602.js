  ...
  reducers: {
    addToCart: (state, { payload }) => {
      ...
      if (!isItemExist) {
        state.cartItems = [
          ...state.cartItems,
          { ...payload, quantityChoose: payload.quantity },
        ];
        ...
        state.cartItems = state.cartItems.map((item) => {
          if (item._id === payload._id) {
            state.quantity += payload.quantity;
            return { ...item, quantityChoose: item.quantity + payload.quantity };
          } else {
            return item;
          }
        });
    ...
    addItemQuantity: (state, { payload }) => {
      state.cartItems = state.cartItems.map((item) => {
        ...
        if (item._id === payload._id) {
          ...
          return { ...item, quantityChoose: item.quantity + 1 };
        } else {
          return item;
        }
      });
